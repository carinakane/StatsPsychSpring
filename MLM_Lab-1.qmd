...
```{r}
#I noticed that the lines are not very well aligned with the dots. Clearly its not fitting well. The lines are also slightly inclined upward but not much.
...
#Only the third model does a good job with some of the really high slopes like sock puppets and minecraft. The second model clearly gets intercepts right, and #first model is very similar to the second it seems. 
...
- What variables are we seeking to investigate in terms of their impact on the outcome?
  times wearing headphones and audio type
...
- What varies *between* these clusters?
how they are listening to stuff
```
...
efdat <- efdat |>
  mutate(
    aud = fct_relevel(factor(audio), "no_audio"),
    hp = fct_relevel(factor(headphones), "S")
  )
...
```{r}
#ENTER YOUR ATTEMPT HERE:
model1 <- lmerTest::lmer(SDMT ~ audio * headphones + (1 + audio | PID), data = efdat)
...
```{r}
#ENTER YOUR ATTEMPT HERE:
emmeans(sdmt_mod, ~ audio * headphones) |>
  as.data.frame() |>
  ggplot(aes(x = audio, y = emmean,
