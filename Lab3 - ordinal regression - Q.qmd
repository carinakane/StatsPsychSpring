...
ggplot(ratings, aes(x = latent)) +
  ...
  geom_vline(xintercept = tau_psych, linetype = "dashed") +
  annotate("text", x = -2,  y = 0.35, label = "Poor") +
  annotate("text", x = -0.5, y = 0.35, label = "Fair") +
  annotate("text", x = 0.5, y = 0.35, label = "Good") +
  annotate("text", x = 2,  y = 0.35, label = "Excellent") +
  labs(title = "Psychology Rating Thresholds") +
  theme_minimal()
...
The way the rating sclaes were presented is different:
"Each instructor has a latent (hidden) "teaching effectiveness" score that we never observe. We only see the ordinal rating. Now, **both departments evaluate the same instructors**, but their evaluation forms use **different descriptions** for the rating categories."
...
```{r}
psych <- ratings %>%
  dplyr::count(eval_psych) %>%
  dplyr::rename(category = eval_psych) %>%
  dplyr::mutate(dept = "Psychology")

cs <- ratings %>%
  dplyr::count(eval_cs) %>%
  dplyr::rename(category = eval_cs) %>%
  dplyr::mutate(dept = "Computer Science")

plot_data <- dplyr::bind_rows(psych, cs) %>%
  dplyr::group_by(dept) %>%
  dplyr::mutate(prop = n / sum(n)) %>%
  dplyr::ungroup()

ggplot(plot_data, aes(x = dept, y = prop, fill = category)) +
...
Each value designated the probability that rating fall within each threshold.
...
```{r}
library(ordinal)
psych_model <- clm(eval_psych ~ 1, data = ratings)
summary(psych_model)
```
...
They are basically the same! This tells me the model, if the model only has an intercept and no predictors, is doing basically what we did in the hand made table 
...
The blue is the actual probability of "fair" P(Fair) and the grey is the cumulative probability of being fair or below. P(Fair<). 
...
Cumulative logit looks at odds of being at that category or below. Basically the grey section above. There are three cumulative logits for the four categories so its K - 1 equations for K categories. This is useful when we are looking at thresholds and probability of being at or belove/above thresholds. 
...
lm(r1 ~ effectiveness, data = ratings)
lm(r2 ~ effectiveness, data = ratings)
```
Report both slopes. In one sentence, explain why they differ and what this reveals about metric models.

The way the numbers are assigned can have drastic effects on the slope. The numbers are arbirary and completely change the model, this is why we dnt use this for categorical data. 
...
```{r}
# Gender
ggplot(gb, aes(Gender, fill = Rank)) +
  geom_bar(position = "fill")

# Age 
gb$AgeGroup <- cut(gb$Age, breaks = c(15, 25, 35, 45, 55, 75))

ggplot(gb, aes(AgeGroup, fill = Rank)) +
  geom_bar(position = "fill")
...
```{r}
model0 <- clm(Rank ~ 1, data = gb)
model1 <- clm(Rank ~ Gender, data = gb)
model2 <- clm(Rank ~ Gender + Age, data = gb)
model3 <- clm(Rank ~ Gender * Age, data = gb)
...
These represent the log-cumulative-odds of a baker being in Rank 3 or better in the first one, and then 2 or better in the second. 
...
age_or <- exp_coef["Age"]
age_percent <- (age_or - 1) * 100
age_or
age_percent
```
The odds ratio increases by every additional year by the amount above. Each additional year of age increases the odds of a better rank by about 0.56%.
...
preds <- predictions(model2,
                     newdata = data.frame(Gender = "F", Age = 30),
                     type  = "prob")
preds
```
...
anova(model2, model3)
```
Yes it is because the likelihood ratio test gives p = 0.035. 
...
```{r}
alpha_3 <- coef(model2)["3|2"]       # threshold for 3|2
beta_gender <- coef(model2)["GenderM"]
beta_age <- coef(model2)["Age"]

# Gender = F (reference level = 0), Age = 25
eta <- alpha_3 - (beta_gender * 0 + beta_age * 25)
prob_rank3_or_less <- plogis(eta)
prob_rank3_or_less
```
------------------------------------------------------------------------
...
```{r}
ggemmeans(m2, terms = c("Age", "Gender")) %>%
  plot() +
  labs(
    x = "Age",
    y = "Predicted Probability",
    color = "Gender",
    title = "Predicted Probability of Rank by Age and Gender"
  )
...
sur_res <- resids(model2)  
qqnorm(sur_res)
qqline(sur_res, col = "red")
...
We used a cumulative logit model (proportional odds) to predict technical rank (1st, 2nd, 3rd) from gender and age (N = 309). A Brant test supported the proportional odds assumption (χ² = [not reported], p = [not reported]). Gender was associated with a slight decrease in the odds of achieving a better rank (OR = 0.83, 95% CI [0.61, 1.13]). Each additional year of age was associated with a slight increase in the odds of achieving a better rank (OR = 1.01, 95% CI [1.00, 1.01]). In terms of predicted probabilities, a 25-year-old female had a 34.6% chance of achieving rank 1 compared with 27.6% for a 25-year-old male, illustrating that switching from female to male decreased the probability of achieving rank 1 by approximately 7 percentage points.
