...
```{r}
# body_mass_g looks roughly symmetric 
# bill_length_mm has a slight right skew,
# Both seem very correlated
...
```{r}
# The residuals should have mean ~0 by construction — OLS minimizes squared
# residuals and the intercept absorbs any constant offset. The SD should
# match the "Residual standard error" from summary(fit1.ols), which is the
# sample estimate of sigma_epsilon in the model equation.

mean(residuals(fit1.ols))
sd(residuals(fit1.ols))

# The SD here corresponds to sigma_epsilon in the LaTeX equation above
# (the noise term around the regression line).
...
```{r}
# lm() talks in frequentist terms: "Estimate", "Std. Error", "t value",
# "Pr(>|t|)". The brm() output drops the t-stats and p-values entirely
# and instead reports "Estimate" (posterior mean), "Est.Error" (posterior SD),
# and credible intervals "l-95% CI" / "u-95% CI". There's no null-hypothesis
# testing language — the output describes a distribution over the parameter,
...
# predict() plot
nd_g <- tibble(body_mass_g = seq(from = min(gentoo$body_mass_g, na.rm = TRUE),
                                 to = max(gentoo$body_mass_g, na.rm = TRUE),
                                 length.out = 50))

predict(fit2.ols, interval = "confidence", newdata = nd_g) %>% 
  data.frame() %>% 
  bind_cols(nd_g) %>% 
  ggplot(aes(x = body_mass_g)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 1/3) +
  geom_line(aes(y = fit)) +
  geom_point(data = gentoo, aes(y = bill_length_mm))
```
...
```{r}
# The diagonal shows the marginal distributions of each parameter. The off-diagonal shows the joint (or bivariate) distributions between pairs of parameters.
...
```{r}
# Each draw is a parameter combination and to get a prediction at some value
# of body mass g, we plug it into the regression formula using each draw's beta0 and beta1, which gives us 4000 predictions instead of one. The
# spread of those predictions is the predictive uncertainty.
...
```{r}
# sigma is constrained to be positive, so the posterior can't extend below 0 but has no upper bound.
...
```{r}
#  Mean can inflates the prediction by pulling toward the long tail and the mode tracks where most of the mass is
# but ignores the tails,  the median is probably best here.
...
```{r}
# fit2.b was already fit in Exercise 1 — reuse it here.
# (Re-subsetting just in case this chunk is run on its own.)
gentoo <- penguins %>% filter(species == "Gentoo")

# pull the draws
draws_g <- as_draws_df(fit2.b) %>% 
  mutate(beta0 = b_Intercept,
         beta1 = b_body_mass_g)
...
```{r, warning = F}
# half-eye at the mean of body_mass_g
draws_g %>%
  mutate(body_mass_g = mean(gentoo$body_mass_g, na.rm = TRUE)) %>% 
  mutate(y_hat = beta0 + beta1 * body_mass_g) %>% 
  ggplot(aes(x = y_hat)) +
  stat_halfeye(point_interval = mean_qi, .width = .95) +
  labs(title = "Gentoo posterior at mean body_mass_g",
       x = expression(hat(italic(y))*'|'*italic(x)==bar(x)))
