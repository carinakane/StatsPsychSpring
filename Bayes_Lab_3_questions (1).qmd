...
```{r}
# The curve gets a lot of the distribution right, its near the mean and stimiar std it seems but tails are way too long 
...
```{r}
#It is the extra noise term that makes the band wider, drawn from Normal( 0, sigma)
...
```{r}
# Same idea as the manual version: pp_check() samples from the posterior and computes
# densities for those simulated datasets, and overlays them on the observed
# density.
```
...
```{r}
# The prior predictive distribution covers the same rough range as the observed bill lengths without
# crazy values. 
```
...
```{r}
# They're basically the same. Thsis means that brms likely fills in priors. I didnt set a priot on all. 
...
```{r}
# body_mass_g is very useful for predicting bill_length_mm.
# The model with the predictor beats the intercept-only model and thats good justification for adding it
```
...
nd_g <- tibble(body_mass_g = seq(
  from = min(gentoo$body_mass_g, na.rm = TRUE),
  to = max(gentoo$body_mass_g, na.rm = TRUE),
  length.out = 100))

fitted(fit1.g, newdata = nd_g) %>% 
  data.frame() %>% 
  bind_cols(nd_g) %>% 
  ggplot(aes(x = body_mass_g)) +
  geom_ribbon(aes(ymin = Q2.5, ymax = Q97.5), alpha = 1/3) +
  geom_line(aes(y = Estimate)) +
  geom_point(data = gentoo, aes(y = bill_length_mm))
```

```{r}
# predict() ribbon (posterior predictive)
predict(fit1.g, newdata = nd_g) %>% 
  data.frame() %>% 
  bind_cols(nd_g) %>% 
  ggplot(aes(x = body_mass_g)) +
  geom_ribbon(aes(ymin = Q2.5, ymax = Q97.5), alpha = 1/3) +
  geom_line(aes(y = Estimate)) +
  geom_point(data = gentoo, aes(y = bill_length_mm)) +
  coord_cartesian(ylim = range(gentoo$bill_length_mm, na.rm = TRUE))
```

```{r}
# pp_check
pp_check(fit1.g, ndraws = 100)
```
