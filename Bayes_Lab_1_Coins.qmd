...
```{r}
#using a similar structure as above, compute row sums, mutate it (and excluding the theta column)
sum_table <- prob_table %>%
  mutate(row_sum = rowSums(across(-theta)))

sum_table

#Every row sums to 1, this tells me that the binomial probabilities sum to 1 when you sum over all outcomes
...
sum_tab <- likelihood_table %>%
  summarise(total_likelihood = sum(likelihood))

sum_tab

#It does not add up to 1, this tells me that the likelihood is not a probability distribution
```
...
The theta that has the highest likelihood is 0.6. You can't call it a probability since it doesn't sum to 1.
...
```{r}
post_sum_tab <- posterior_table %>%
  summarise(total_likelihood = sum(posterior))

post_sum_tab

#This tells me that the posterior is a p distribution now over possible thetas
```
...
print("there's a 0.3019659% chance that theta is 0.6")
posterior_table$posterior[ posterior_table$theta == 0.6]

print("there's a  0.5750109% chance that theta is 0.6")
posterior_table$posterior[ posterior_table$theta == 0.5] + posterior_table$posterior[ posterior_table$theta == 0.6]

# There's a % chance that $\theta$ is 0.6
```
...
The posterior is now favoring 0.5, the prior definitely pulled the estimate closer to 0.5 and brought in the tails.
...
The posterior shifted because the prior multiplied the linekihood differently and the prior was already favoring a certain kind of probability distribution but allowed itself to be updated.
...
The peak is at about 0.62 or something. This is essentially what we saw before but its more precise.
...
The posterior is there because the posterior takes both into account and then adjusts the estimate. They get multiplied together. So its a compromise between both. If we saw 300 and 500 then the posterior would be nearly exactly the likelihood since the prior would not be really informative at that point.
...
As n increases, the posterior moves towards the likelihood. the prior doesn't really matter at n=200.
...
The posterior mean in nearly 0.6, with a nearly uniform prior, the posterior is going to be close to the likelihood.
...
This is very close to the grid approximation with a flat prior.
...
It is very similar in shape but the y axis is different. Here it is density, whereas the previous one is posterior probability and I assume the area under the curve sums to 1 in the previous table. This one is also chunkier and doesnt have smooth tails like the one in section 5.
...
This shifted towards the prior which favored a fair coin. So it moved towards 0.5 from the previous section. This does match what happened in section 4.
...
The prior would start to become negligible and the curves would be centered around 0.6.
...
In lecture, the blank was: "There's a \_\_\_% chance that $\theta$ is 0.6." With continuous posterior draws, we can't ask about a single point. Instead, we ask about intervals. Compute: "There's a 16.99% chance that $\theta$ is between 0.55 and 0.65." How does this compare to the grid approximation posterior at $\theta = 0.6$?

If I understand the question correctly, this should be the sum of the area under the curve in the plots we made between the interval. If by grid you mean table, its allowing a lot more flexibility than the tables because it allows us to look at any interval, we can no longer estimate for particular points.
...
Yes and yes. It tells me maybe we need more samples because that's a huge range.
...
The histogram has density as the y and the posterior table is a probability distribution and the y is the probability. Both of them represent the posterior distribution. The posterior table is for point estimates and the histogram is looking at continuous values and is better for looking at intervals.
